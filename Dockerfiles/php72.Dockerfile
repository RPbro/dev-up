...
RUN set -x \
    ...
    && apk update \
    && apk add bash vim git autoconf gcc g++ make pcre-dev re2c libstdc++ tzdata python libressl2.7-libcrypto openssl-dev zlib-dev linux-headers freetype-dev libjpeg-turbo-dev libpng-dev libzip-dev \
    ...
    && apk del autoconf gcc g++ make pcre-dev re2c tzdata python openssl \
    && rm -rf /tmp/pkg
