...
RUN set -x \
    ...
    && apk update \
    && apk add bash vim git autoconf gcc g++ make pcre-dev re2c libstdc++ tzdata python openssl zlib-dev linux-headers freetype-dev libjpeg-turbo-dev libpng-dev libzip-dev \
